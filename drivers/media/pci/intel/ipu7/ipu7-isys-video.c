...
ipu7_isys_get_isys_format(u32 pixelformat, u32 type)
{
	const struct ipu7_isys_pixelformat *default_pfmt = NULL;
	unsigned int i;
				     ...
				     struct v4l2_capability *cap)
{
	...
	snprintf(cap->bus_info, sizeof(cap->bus_info), "%s",
		 to_isys7(av)->ipu.media_dev.bus_info);
				     ...
				     struct v4l2_fmtdesc *f)
{
	...
	for (i = 0, num_found = 0; i < ARRAY_SIZE(ipu7_isys_pfmts); i++) {
		if ((ipu7_isys_pfmts[i].is_meta &&
		     f->type != V4L2_BUF_TYPE_META_CAPTURE) ||
		    (!ipu7_isys_pfmts[i].is_meta &&
				  ...
				  u32 *bytesperline, u32 *sizeimage)
{
	const struct ipu7_isys_pixelformat *pfmt =
		ipu7_isys_get_isys_format(*format, type);
					   ...
					   struct v4l2_format *f)
{
	...
	ipu7_isys_vidioc_try_fmt_meta_cap(file, fh, f);
				 ...
				 struct ipu_isys_buffer_list *bl)
{
	...
	list_for_each_entry(aq, &stream->queues, node) {
		...
		if (ret < 0) {
			ipu7_put_fw_msg_buf(to_isys7(av),
	...
	ret = ipu7_fw_isys_complex_cmd(to_isys7(av), stream->stream_handle,
				       stream_cfg, msg->dma_addr,
				       sizeof(*stream_cfg),
				       IPU_INSYS_SEND_TYPE_STREAM_OPEN);
	if (ret < 0) {
		dev_err(dev, "can't open stream (%d)\n", ret);
		ipu7_put_fw_msg_buf(to_isys7(av), (uintptr_t)stream_cfg);
		return ret;
	}
...
out_stream_close:
	...
	retout = ipu7_fw_isys_simple_cmd(to_isys7(av), stream->stream_handle,
...
static void stop_streaming_firmware(struct ipu7_isys_video *av)
{
	...
	ret = ipu7_fw_isys_simple_cmd(to_isys7(av), stream->stream_handle,
...
static const struct v4l2_ioctl_ops ipu7_v4l2_ioctl_ops = {
	...
	.vidioc_enum_fmt_meta_cap = ipu7_isys_vidioc_enum_fmt,
	...
	.vidioc_s_fmt_meta_cap = ipu7_isys_vidioc_s_fmt_meta_cap,
...
int ipu7_isys_video_init(struct ipu7_isys_video *av)
{
	...
	av->vdev.v4l2_dev = &to_isys7(av)->ipu.v4l2_dev;
	av->vdev.dev_parent = &to_isys7(av)->ipu.adev->isp->pdev->dev;
	...
	__ipu7_isys_vidioc_try_fmt_meta_cap(av, &format_meta);
